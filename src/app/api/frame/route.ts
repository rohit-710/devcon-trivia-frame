...
async function getResponse(req: NextRequest): Promise<NextResponse> {
  ...
  const id = parseInt(idString) || 1; // Start with id=1 if not provided
  const nextId = id + 1;
  ...
  // Display the question frame initially
  if (!data || !data.untrustedData || typeof buttonId === "undefined") {
    return new NextResponse(`
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <meta property="fc:frame:image:aspect_ratio" content="1.91:1" />
        <meta property="fc:frame:post_url" content="${NEXT_PUBLIC_URL}/api/frame?id=${id}" />
        </head>
        </html>`);
  }
  ...
  // In case of unexpected scenario, re-display the current question
  return new NextResponse(`
      <!DOCTYPE html>
      <html>
      <head>
