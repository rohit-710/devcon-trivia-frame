...
async function getResponse(req: NextRequest): Promise<NextResponse> {
  ...
  if (id > 1 && buttonId - 1 !== correctAnswers[id - 2]) {
    ...
    <meta property="fc:frame:post_redirect" content="${process.env.NEXT_PUBLIC_URL}/api/frame?id=1" />
  ...
  if (id === 4) {
    ...
    <meta property="fc:frame:button:1" content="Play again"} />
    <meta property="fc:frame:post" content="${process.env.NEXT_PUBLIC_URL}/api/frame?id=1" />
    <meta property="fc:frame:image:aspect_ratio" content="1.91:1" />
  </head></html>`);
  } else {
    return new NextResponse(`<!DOCTYPE html><html><head>
